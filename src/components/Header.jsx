const Header = () => {
    return (
        <div className="flex flex-row justify-between h-16 px-2 py-2 pl-3 items-center">
            <div className="flex flex-row w-1/12 items-center h-full" >
                <img alt="Sidebar-icon" className="w-6 h-3/4 mx-2 " src="https://imgs.search.brave.com/jJtUaMpEKfzsgb9_JVG05f3X5hXfFyvi0yqwa4NFtQ8/rs:fit:500:0:0:0/g:ce/aHR0cHM6Ly9pY29u/cy52ZXJ5aWNvbi5j/b20vcG5nL28vbWlz/Y2VsbGFuZW91cy9i/aWctZGF0YS1yZWd1/bGFyLW1vbm9jaHJv/bWUtaWNvbi9zaWRl/YmFyLTQucG5n"/>
                <img alt="youtube-icon" className="w-14 ml-2 h-full " src="data:image/png;base64," />
            </div> 
            <div className="flex flex-row w-full h-full ml-2 justify-center items-center" >
                <input type="text" placeholder="Search" className="border-2 pb-1 pl-6 items-center flex justify-center text-xl my-auto w-5/12 h-full rounded-l-full border-gray-300" />
                <button className="p-2 rounded-r-full h-full hover:bg-gray-200 bg-gray-100 border-r-2 border-y-2 border-left-0 border-gray-300" >Search</button>
                <div className="w-11 h-12 ml-4 rounded-full bg-gray-100 flex items-center justify-center" >
                    <img className="w-7 h-7 hover:cursor-pointer" src="https://www.iconpacks.net/icons/1/free-microphone-icon-342-thumb.png" alt="voice search" />
                </div>
            </div>
            <div className="flex flex-row w-1/12 h-full items-center justify-between" >
                <img className="w-8 h-3/5  hover:cursor-pointer" src="https://www.iconpacks.net/icons/4/free-notification-bell-icon-13081.png" alt="notification-icon" />
                <img className="w-8 h-3/5 hover:cursor-pointer" src="https://www.iconpacks.net/icons/2/free-icon-video-camera-2806.png" alt="create-icon" />
                <img className="w-9 h-5/6 mr-1 hover:cursor-pointer"  src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRtRs_rWILOMx5-v3aXwJu7LWUhnPceiKvvDg&s" alt="user-icon" />
            </div>

        </div>
    )
}

export default Header;